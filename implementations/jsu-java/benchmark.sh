#! /bin/sh
#
# JSON Schema Benchmark with JSU Java
#
...
if [ $# -eq 0 ] || [ "$1" = "--help" -o "$1" = "-h" ] ; then
    ...
    echo "jsu $(jsu-compile --version) [Java $(java --version|head -1)]"
    exit 0
elif [ $# -eq 1 ] ; then
    dir=$1
...
CLASSPATH="$appdir:$workdir:$CLASSPATH"
...
jsu-compile \
    ...
    -o "$workdir/Schema.java" \
    "$SCHEMA" \
    -- \
    --quiet

javac -encoding "utf-8" JsonSchemaBenchmark.java

compile_end=$(date +%s%N)
compile_time=$(( $compile_end - $compile_start ))
msg "$SCHEMA compile time is $(( $compile_time / 1000 )) µs"

java JsonSchemaBenchmark "$INSTANCES"
